...
# --- КОНСОЛЬ РАЗРАБОТЧИКА В ТЕРМИНАЛЕ ---
async def console_input_loop(bot: Bot):
    loop = asyncio.get_event_loop()
    print("\n💻 Консоль разработчика готова!")
    print("Формат выдачи: <юзернейм_или_id> <сумма>")
    print("Пример: @username 500 или 123456789 500\n")

    while True:
        try:
            input_str = await loop.run_in_executor(None, sys.stdin.readline)
            if not input_str or not input_str.strip():
                continue

            parts = input_str.strip().split()
            if len(parts) < 2:
                print("⚠️ Ошибка ввода! Формат: <юзернейм_или_id> <сумма>")
                continue
            ...
            try:
                ...
                print("⚠️ Ошибка: Сумма должна быть целым числом!")
                continue

            user_id = None
            first_name = "Игрок"

            # 1. Если введен ID числом
            if target.isdigit():
                user_id = int(target)
            else:
                # 2. Поиск пользователя в локальной базе по юзернейму
                cursor.execute('SELECT user_id FROM users WHERE LOWER(username) = LOWER(?)', (target,))
                row = cursor.fetchone()
                if row:
                    user_id = row[0]
                else:
                    # 3. Попытка запроса у Telegram API
                    try:
                        ...
                        print(
                            f"❌ Пользователь @{target} не найден! (Убедитесь, что он написал хотя бы одно сообщение в чат с ботом)")
                        continue
            ...
            print(f"✅ Выдано {amount} 💎 пользователю (ID: {user_id}). Новый баланс: {new_bal}")
            ...
            try:
                ...
                print(f"📩 Уведомление отправлено пользователю {first_name}!")
            except Exception as e:
                print(f"⚠️ Не удалось отправить сообщение в ЛС (пользователь не за Notified / заблокировал бота): {e}")

        except Exception as e:
            print(f"❌ Ошибка в консоли разработчика: {e}")
...
async def play_game(message: Message):
    ...
    # Безопасная отправка ответа (с предотвращением TelegramBadRequest)
    try:
...
async def main():
    print("Запуск бота...")
    asyncio.create_task(console_input_loop(bot))
